  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#000000cc", zIndex:999, display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"#111", border:"1px solid #333", borderRadius:16, padding:24, width:300, fontFamily:"monospace" }}>
        <div style={{ color:"#888", fontSize:10, letterSpacing:2, marginBottom:12 }}>MISE À JOUR SCORE</div>
  ...
  return (
    <svg width={size} height={size} style={{ transform:"rotate(-90deg)", flexShrink:0 }}>
      ...
      <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={color} strokeWidth={5}
        strokeDasharray={circ} strokeDashoffset={circ*(1-Math.max(0,value)/100)}
        strokeLinecap="round" style={{ transition:"stroke-dashoffset 0.7s" }} />
      <text x={size/2} y={size/2} textAnchor="middle" dominantBaseline="central"
        fill={color} fontSize={size<60?11:13} fontWeight={900} fontFamily="monospace"
...
function EdgeMeter({ edgePct }) {
  const c=Math.max(-30,Math.min(30,edgePct||0)), pct=((c+30)/60)*100, pos=(edgePct||0)>0, col=pos?"#00ff88":"#ff2244";
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
        <span style={{ color:"#ff4444", fontSize:9, fontFamily:"monospace" }}>◄ UNDER</span>
        <span style={{ color:"#777", fontSize:9, fontFamily:"monospace" }}>EDGE: {edgePct>0?"+":""}{edgePct}%</span>
        <span style={{ color:"#00ff88", fontSize:9, fontFamily:"monospace" }}>OVER ►</span>
      </div>
      <div style={{ position:"relative", height:10, borderRadius:5, background:"#111" }}>
        <div style={{ position:"absolute", left:"50%", top:-3, width:1, height:16, background:"#2a2a2a" }} />
        <div style={{ position:"absolute", left:0, top:0, height:"100%", width:`${pct}%`, background:pos?"linear-gradient(90deg,#1a1a1a 50%,#00ff88)":"linear-gradient(90deg,#ff2244,#1a1a1a 50%)", borderRadius:5, transition:"width 0.7s" }} />
        <div style={{ position:"absolute", top:-4, left:`calc(${pct}% - 4px)`, width:8, height:18, background:col, borderRadius:2, boxShadow:`0 0 8px ${col}`, transition:"left 0.7s" }} />
      </div>
    </div>
  );
}
      ...
      <div onClick={()=>onSelect(game.id)} style={{ background:selected?"#0b130f":"#090909", border:`1px solid ${selected?c+"77":"#181818"}`, borderRadius:13, padding:"15px 17px", cursor:"pointer", transition:"all 0.2s", boxShadow:selected?`0 0 18px ${c}14`:"none" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:9 }}>
          ...
          <div style={{ textAlign:"center" }}>
            <div style={{ color:"#fff", fontFamily:"monospace", fontSize:20, fontWeight:900, letterSpacing:-1 }}>{game.isLive ? `${game.awayScore}–${game.homeScore}` : "vs"}</div>
        ...
        {!game.isLive && <div style={{ color:"#222", fontSize:9, fontFamily:"monospace" }}>Tap ✏️ pour entrer le score en direct</div>}
      </div>
    </div>
  );
}
...
function DetailPanel({ game }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      <div style={{ background:`${c}08`, border:`1px solid ${c}28`, borderRadius:15, padding:"18px 22px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div>
          <div style={{ color:"#333", fontSize:9, fontFamily:"monospace", letterSpacing:2, marginBottom:5 }}>SIGNAL</div>
          <div style={{ color:c, fontFamily:"monospace", fontSize:22, fontWeight:900 }}>{sig.label}</div>
          <div style={{ color:"#444", fontFamily:"monospace", fontSize:11, marginTop:3 }}>{game.awayFull} @ {game.homeFull}</div>
        </div>
        <Ring value={sig.confidence} color={c} size={68} />
      </div>
      {game.isLive && game.total ? (
        <>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:9 }}>
            {[
              { label:"PROJETÉ", val:sig.projection, color:c, big:true },
              { label:"LINE", val:game.total, color:"#aaa" },
              { label:"EDGE", val:`${sig.edge>0?"+":""}${sig.edge}`, color:sig.edge>3?"#00ff88":sig.edge<-3?"#ff4444":"#888" },
            ].map((m,i)=>(
              <div key={i} style={{ background:"#0d0d0d", border:"1px solid #181818", borderRadius:11, padding:"13px 15px" }}>
                <div style={{ color:"#333", fontSize:8, fontFamily:"monospace", marginBottom:3 }}>{m.label}</div>
                <div style={{ color:m.color, fontFamily:"monospace", fontSize:m.big?26:20, fontWeight:900 }}>{m.val}</div>
              </div>
            ))}
          </div>
          <div style={{ background:"#0d0d0d", border:"1px solid #181818", borderRadius:11, padding:"15px 18px" }}>
            <div style={{ color:"#333", fontSize:8, fontFamily:"monospace", letterSpacing:2, marginBottom:10 }}>ANALYSE</div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8 }}>
              {[{ label:"MARQUÉ", val:scored },{ label:"PACE/MIN", val:sig.pacePerMin },{ label:"MIN. REST.", val:sig.remaining }].map((m,i)=>(
                <div key={i} style={{ textAlign:"center" }}>
                  <div style={{ color:"#333", fontSize:8, fontFamily:"monospace" }}>{m.label}</div>
                  <div style={{ color:"#fff", fontFamily:"monospace", fontSize:16, fontWeight:700 }}>{m.val}</div>
                </div>
              ))}
            </div>
          </div>
          <div style={{ background:"#080808", border:`1px solid ${c}1a`, borderRadius:11, padding:"15px 18px" }}>
            <div style={{ color:"#ffd700", fontFamily:"monospace", fontSize:13, fontWeight:700, marginBottom:10 }}>
            ...
            <a href="https://polymarket.com/sports/basketball/nba" target="_blank" rel="noopener noreferrer"
              style={{ display:"inline-flex", alignItems:"center", gap:6, background:"linear-gradient(135deg,#7c3aed,#5b21b6)", color:"#fff", textDecoration:"none", borderRadius:7, padding:"8px 15px", fontFamily:"monospace", fontSize:11, fontWeight:700 }}>
              🔗 OUVRIR POLYMARKET
            </a>
          </div>
        </>
      ) : (
        <div style={{ background:"#0d0d0d", border:"1px solid #181818", borderRadius:11, padding:"30px", textAlign:"center" }}>
          <div style={{ fontSize:36, marginBottom:10 }}>🏀</div>
          <div style={{ color:"#444", fontFamily:"monospace", fontSize:12 }}>Match prévu à {game.time}</div>
          <div style={{ color:"#222", fontFamily:"monospace", fontSize:10, marginTop:8 }}>Tap ✏️ pour entrer le score en direct</div>
        </div>
      )}
    </div>
  );
...
export default function App() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#060606", color:"#fff", fontFamily:"monospace" }}>
      ...
      <div style={{ borderBottom:"1px solid #0f0f0f", padding:"14px 26px", display:"flex", justifyContent:"space-between", alignItems:"center", background:"#060606", position:"sticky", top:0, zIndex:100 }}>
        <span style={{ background:"linear-gradient(135deg,#7c3aed,#00ff88)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", fontSize:20, fontWeight:900, letterSpacing:3 }}>NBA × POLYMARKET</span>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"360px 1fr", height:"calc(100vh - 53px)" }}>
        <div style={{ borderRight:"1px solid #0f0f0f", overflowY:"auto", padding:"14px 12px", display:"flex", flexDirection:"column", gap:9 }}>
          {liveGames.length > 0 && <><div style={{ color:"#ff4444", fontSize:8, letterSpacing:3 }}>● LIVE</div>{liveGames.map(g=><GameCard key={g.id} game={g} selected={selectedId===g.id} onSelect={setSelectedId} onEdit={setEditGame} />)}</>}
          <div style={{ color:"#333", fontSize:8, letterSpacing:3, marginTop:8 }}>CE SOIR — 6 MARS</div>
        ...
        <div style={{ overflowY:"auto", padding:"18px 22px" }}>
          {selected && <DetailPanel game={selected} />}
        </div>
      </div>
    </div>
  );
